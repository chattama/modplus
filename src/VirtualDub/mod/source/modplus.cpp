	...
	case WM_VDM_REQUESTFRAMESIZE:
		{
			...
			fs->frame.bottom = 0;
			if (inputVideo) {
				VDAVIBitmapInfoHeader *formatIn = inputVideo->getImageFormat();

				if (formatIn) {
					VDAVIBitmapInfoHeader *dcf = inputVideo->getDecompressedFormat();
					switch (fs->frametype) {
						case 0: {
						...
						case 1: {
							if (!filters.isRunning()) {
								...
								filters.prepareLinearChain(&g_listFA, dcf->biWidth, dcf->biHeight, 16+8*g_dubOpts.video.mOutputFormat, pVSS->getRate(), pVSS->getLength(), inputVideo->getPixelAspectRatio());
							}
							...
							while(fa_list->next) {
								x += fa_list->GetCropInsets().left;
								y += fa_list->GetCropInsets().top;
								fa_list = (FilterInstance *)fa_list->next;
							}
							
							fs->frame.left = x;
							fs->frame.top = y;
							fs->frame.right = filters.GetOutputLayout().w;
							fs->frame.bottom = filters.GetOutputLayout().h;
						}
						default: break;
					}
				}
...
void AVSViewerLoadSettings(HWND hwnd) {
	if (!hwnd) return;
	VDUIRestoreWindowPlacementW32(hwnd, REG_KEY_SCRIPTEDITOR, SW_SHOWNORMAL);
}
